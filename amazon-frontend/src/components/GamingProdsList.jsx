...
function GamingProdsList(props) {
...
let products = [...originalProds]
  ...
  useEffect(() => {
    if(filterData){
      fetch(`https://amazon-clone-backend-mxip.onrender.com/filter/gamingProducts/${filterData}`).then((response)=>response.json()).then((data)=>{
        setProds(data)
      }).catch((err)=>{
        console.log(err)
      })
      ...
      fetch(`https://amazon-clone-backend-mxip.onrender.com/filterRating/gamingProducts/${filterRatingData}`).then((response)=>response.json()).then((data)=>{
        setProds(data)
      }).catch((err)=>{
        console.log(err)
      })
    ...
    fetch("https://amazon-clone-backend-mxip.onrender.com/gamingProducts")
      .then((response) => 
        response.json()
      )
      .then((data) => {
    ...
    <>
      ...
      <div id="mobileFilters" className="bg-white  transition-all  duration-500 fixed h-full top-0 -left-[100%] w-full lg:hidden">
       ...
       mobileFilters.style.left="-100%"
        ...
        <div className="grid grid-cols-[130px_1fr] grid-rows-[700px_500px]  cursor-pointer shadow">
          ...
          <div id="FilByPrice" className="bg-gray-100 p-4 mt-4 rounded ">
            ...
            <div className="flex flex-col mt-2">
              ...
              <label className="flex items-center space-x-2 pt-1">
                <input type="checkbox" value="2000" onChange={(e)=>{
                    if(e.target.checked){
                      setFilterData(e.target.value)
                    }
                    else{
              ...
              <label className="flex items-center space-x-2 pt-1">
                <input type="checkbox" value="5000" onChange={(e)=>{
                    if(e.target.checked){
                      setFilterData(e.target.value)
                    }
                    else{
              ...
              <label className="flex items-center space-x-2 pt-1">
                <input type="checkbox" value="30000" onChange={(e)=>{
                    if(e.target.checked){
                      setFilterData(e.target.value)
                    }
                    else{
          ...
          <div className=" w-full h-10 mt-5 pr-8 flex justify-end">
          <button className="bg-blue-600 w-[70%] h-10 p-2 font-semibold rounded-md text-white cursor-pointer" onClick={()=>{
        let mobileFilters=document.getElementById("mobileFilters") 
       mobileFilters.style.left="-100%"
...
export default GamingProdsList