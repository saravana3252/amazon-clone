...
function ClothingProdsList(props) {
...
let products = [...originalProds]
  ...
  useEffect(() => {
    if(filterData){
      fetch(`https://amazon-clone-backend-mxip.onrender.com/filter/clothing/${filterData}`).then((response)=>response.json()).then((data)=>{
        setProds(data)
      }).catch((err)=>{
        console.log(err)
      })
      ...
      fetch(`https://amazon-clone-backend-mxip.onrender.com/filterRating/clothing/${filterRatingData}`).then((response)=>response.json()).then((data)=>{
        setProds(data)
      }).catch((err)=>{
        console.log(err)
      })
    ...
    fetch("https://amazon-clone-backend-mxip.onrender.com/clothingProducts")
      .then((response) => 
        response.json()
      )
      .then((data) => {
      ...
      <div id="mobileFilters" className="bg-white  transition-all  duration-500 fixed min-h-screen top-0 -left-[100%] w-full lg:hidden">
       ...
       mobileFilters.style.left="-100%"
        ...
        <div className="grid grid-cols-[130px_1fr] md:grid-cols-[200px_1fr]  cursor-pointer shadow">
          ...
          <div id="FilByPrice" className="bg-gray-100 p-4 mt-4 rounded ">
            ...
            <div className="flex flex-col mt-2">
              <label className="flex items-center space-x-2">
                <input type="checkbox" value="350" onChange={(e)=>{
                    if(e.target.checked){
                ...
                <span>Less than Rs 350</span>
              </label>
              <label className="flex items-center space-x-2 pt-1">
                <input type="checkbox" value="400" onChange={(e)=>{
                    if(e.target.checked){
                      setFilterData(e.target.value)
                    }
                    else{
              ...
              <label className="flex items-center space-x-2 pt-1">
                <input type="checkbox" value="800" onChange={(e)=>{
                    if(e.target.checked){
                      setFilterData(e.target.value)
                    }
                    else{
          ...
          <div  id="FilByCat" className="bg-gray-100 p-2 lg:p-4 mt-5 rounded hidden">
            ...
            <p className="flex justify-between"><Link to="/GamingProducts">Gaming</Link> <span className="text-gray-400">(10 products)</span></p>
              ...
              <p className="flex justify-between"><Link to="/HomeProducts">Home-prods</Link> <span className="text-gray-400">(10 products)</span></p>
...
export default ClothingProdsList